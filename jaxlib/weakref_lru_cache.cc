...
class WeakrefLRUCache : public std::enable_shared_from_this<WeakrefLRUCache> {
 ...
 private:
  ...
  void EvictLeastRecentlyUsed() {
    ...
    // exist
    CHECK(cache_it != entries_.end());
    auto inner_it = cache_it->second->find(tail->key);
    if (inner_it != cache_it->second->end()) {
      cache_it->second->erase(inner_it);
    }
  }
...
PyObject* WeakrefLRUCache::Call(PyObject* self_obj,
...

  {
    ...
    std::shared_ptr<Cache> cache_ptr = it_weak->second;
    Cache& cache = *it_weak->second;
